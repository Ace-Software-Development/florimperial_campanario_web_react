...
export default function ReservationsClinicas(props) {
  ...
  const handleEditClick = event => {
    ...
    clinicsData.forEach(clinic => {
      if (clinic.objectId === event.target.value) clinicObj = clinic;
    });
          ...
          {clinicsData.map(row => {
            return (
              <tr key={`${row.objectId}-tr`}>
                ...
                <td key={`${row.objectId}-td-5`}>
                  {Object.keys(row.dias).map(x => (row.dias[x] ? x.slice(0, 2) + ' ' : ''))}
                </td>
